...
- ~~Медленная загрузка~~ — загружает 500 тикетов → **теперь 20**
- ~~Пагинация на клиенте~~ → **теперь серверная**
- ~~Нет фильтрации на бэке~~ → **теперь статус/категория фильтруются на сервере**
- Поиск и даты — локальная фильтрация (допустимо)
...
### 2026-06-12 — Оптимизация дашборда

- Уменьшена загрузка тикетов (500 → 20)
- Добавлена серверная фильтрация по статусу и категории
- Исправлена пагинация (использует total с сервера)
- Добавлены переменные `serverTotal` и `serverTotalPages`
