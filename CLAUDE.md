...
| GET | `/api/supports` | Список всех саппортов |
...
- Фильтр по саппорту, статистика саппорта
- Расписание дежурных на неделю
- Показ смены (день/ночь)
- **TODO:** Realtime обновления (WebSocket/SSE)
...
### 2026-06-12 — Улучшения дашборда и фильтрация

- **Карточка дежурных** — показывает текущую смену (день/ночь), время
- **Расписание на неделю** — новый эндпоинт `/api/schedule`, отображение в дашборде
- **Фильтр по саппорту** — dropdown в дашборде, статистика выбранного саппорта
- **Эндпоинт `/api/supports`** — список всех саппортов из тикетов
